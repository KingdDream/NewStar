<template>
    <div class="conversationLeft">
        <div class="search-list">
            <div>
                <span>级别：</span>
                <el-select v-model="value" placeholder="全部" @change="dateVa">
            ...
            <div>
                <span>日期：</span>
                <el-date-picker v-model="value1" type="date" value-format="yyyy-MM-dd" @change="dateValue"
                    placeholder="选择日期"></el-date-picker>
            </div>

            <div>
                <button v-if="this.deleted.length>0?true:false" class="btn-bt" @click="cancel()">批量确认</button>
            </div>
        </div>
...
<script>
    import { selByTerm, isRead, isDone, selLogs,toExcelWaring } from "../../request/api"
    export default {
        data() {
            return {
                options: [
        ...
        methods: {
            ...
            isRead() {
                ...
                isRead(data).then(res => {
                    if (res.result >= 0) {
                        this.$bus.emit("AlertA", res.result);
                    }
                })
            },
            isDone(id) {
